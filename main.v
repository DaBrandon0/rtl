...
/*
assign data_bus = we ? data_out_ctrl : 'bz; // 1st driver of the data bus -- tri state switches

assign data_bus = we ? 'bz : data_out_mem;
...
CMU clkmanage(clk, clear, ssp_intr_i, phione, phitwo, clkout, clearout);
MASTER wishmaster(
    clear,
    clk,
//from ARM
    mem_req_bar, //0 = memory request
    mwr_arm,
    r,
    w,
    addressBus,
//from slave
    dat_i,// recieves binary data
    ack_i, //acknowledges termination of normal bus cycle
    tagn_i,
    adr_o,// passes binary address
    dat_o,// passes binary data
    we_o,// bus cycle is read(0) or write(1)
    stb_o,// indicates valid data transfer, 0 when reset
    cyc_o,// 1 = valid bus cycle in progress, 0 when reset
    tagn_o,
    dataBus2
    );
...
wire [7:0] PRdataBus;
    ...
    assign dataBus = ssp_sel_o && !ssp_w_o ?  PRdataBus: 32'bz;
